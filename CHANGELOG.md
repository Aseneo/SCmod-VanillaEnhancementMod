...
- **模组武器兼容系统**: 三级检测（类型继承 → behavior 绑定 → 反射方法签名）自动识别非原版武器并适配装填模式。
- **弹药兼容系统**: 优先使用官方 `behavior.GetProcessInventoryItemCapacity` + `ProcessInventoryItem` API，兜底反射弹药块 `Get*Type` 方法签名，兼容自定义模组弹药。
- **装填状态持久记忆**: 首次确认装填后跨按键保持，满弹反复按 R 不再误报。
...
- **弩拉弦**: 一步拉满至 15，模组多发弩由 behavior 控制逐发装填。
